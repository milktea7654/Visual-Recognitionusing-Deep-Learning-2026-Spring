...
def verify_parameter_constraint(
        model: torch.nn.Module,
        max_params: int = 100e6) -> bool:
    ...
    ok = '✓ PASS' if within_limit else '✗ FAIL'
    print(f"Parameter Constraint: {ok} (< {max_params/1e6:.0f}M)")
...
class RandAugment:
    ...
    def _apply_operation(self, img: Image.Image, op: str,
                         magnitude: float) -> Image.Image:
        ...
        if op == 'ShearX':
            ...
            img = img.transform(
                (w, h), Image.AFFINE, (1, magnitude, 0, 0, 1, 0))

        elif op == 'ShearY':
            magnitude = np.clip(magnitude / 30 * 0.3, 0, 0.3)
            img = img.transform(
                (w, h), Image.AFFINE, (1, 0, 0, magnitude, 1, 0))

        elif op == 'TranslateX':
            magnitude = int(magnitude / 30 * w * 0.3)
            img = img.transform(
                (w, h), Image.AFFINE, (1, 0, magnitude, 0, 1, 0))

        elif op == 'TranslateY':
            magnitude = int(magnitude / 30 * h * 0.3)
            img = img.transform(
                (w, h), Image.AFFINE, (1, 0, 0, 0, 1, magnitude))
