...
Generates a CSV file with format:
    image_id, class_id
"""
...
class NumericSortedImageFolder(datasets.ImageFolder):
    ...
    Ensures folder '10' maps to index 10, not index 2.
    Crucial for correct class-to-folder mapping in this dataset.
    """
    
    def find_classes(self, directory):
        """Find classes and sort them numerically."""
        classes = sorted(
...
class SimpleImageDataset(Dataset):
    """Dataset for loading images without class folders (for test set)."""
    ...
    if model_name in ['ecaresnet50d', 'gcresnet50t', 'resnet101']:
        if model_name in ['ecaresnet50d', 'gcresnet50t']:
            if not TIMM_AVAILABLE:
                raise ImportError(f"timm is required for {model_name}. Install with: pip install timm")
        ...
        raise ValueError(f"Unsupported model: {model_name}. Choose from: resnet50_inat, resnext101_32x8d, resnet101, ecaresnet50d, gcresnet50t")
    ...
    if os.path.exists(checkpoint_path):
        ...
        try:
            ...
            if "se.fc" in str(e) or "_se_block_cache" in str(e):
                print("Detected SE Block parameters in checkpoint, adding SE blocks...")
                import sys
...
def generate_predictions(model, test_loader, device, output_path: str = "prediction.csv"):
