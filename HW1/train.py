...
from config import *
from utils import (
    setup_logging, get_num_parameters, verify_parameter_constraint,
    create_directories, save_checkpoint, load_checkpoint, set_seed,
    AverageMeter, accuracy, get_advanced_transforms, CutMix, apply_cutmix_to_batch
)
...
class ResNetClassifier:
    ...
    def __init__(self, model_name: str = "ecaresnet50d", num_classes: int = 100, 
                 pretrained: bool = True, weights: str = "IMAGENET1K_V1", device: str = "cuda"):
        ...
        if model_name in ['resnet50_inat', 'resnext101_32x8d']:
            if model_name == 'resnet50_inat':
                try:
                    ...
                    print(f"✓ Loading ResNet50 with iNaturalist pretrained weights")
                except:
                    self.model = models.resnet50(weights=None)
                    print(f"✓ Loading ResNet50 (iNat weights unavailable, using ImageNet-1K)")
            else: 
                self.model = models.resnext101_32x8d(weights='DEFAULT')
                print(f"✓ Loading ResNeXt-101 32x8d with ImageNet-1K weights")
            ...
            weight_info = {
                ...
                'ecaresnet50d': 'RA-Augmented ImageNet (best for this architecture)',
                'gcresnet50t': 'ImageNet-1K'
            }
            
            if pretrained:
                weights_desc = weight_info.get(model_name, 'ImageNet-1K')
                print(f"✓ Loading {model_name} from timm with pretrained weights")
                print(f"  Weights: {weights_desc}")
            
            self.model = timm.create_model(timm_name, pretrained=pretrained, num_classes=num_classes)
        else:
            raise ValueError(f"Unsupported model: {model_name}. Use: resnet50_inat, resnext101_32x8d, resnet101, ecaresnet50d, or gcresnet50t")
        ...
        print(f"Parameter Constraint (< 100M): {'✓ PASS' if total_params < 100e6 else '✗ FAIL'}")
        print(f"{'='*60}\n")
        
        if total_params >= 100e6:
            raise ValueError(f"Model exceeds 100M parameter limit: {total_params/1e6:.2f}M")
...
def get_data_transforms():
    ...
    val_test_list.append(transforms.Resize((resize_size, resize_size), antialias=True))
...
def get_dataloaders(batch_size: int = 64, num_workers: int = 4, use_advanced_aug: bool = True):
    
    if use_advanced_aug:
        train_transform, val_test_transform = get_advanced_transforms(image_size=224)
        print("✓ Using advanced augmentations: RandomResizedCrop + RandAugment + ColorJitter")
    else:
        train_transform, val_test_transform = get_data_transforms()
        print("✓ Using standard augmentations")
    
    train_dataset = NumericSortedImageFolder(TRAIN_DIR, transform=train_transform)
    val_dataset = NumericSortedImageFolder(VAL_DIR, transform=val_test_transform)
...
def get_scheduler(optimizer: optim.Optimizer, num_epochs: int, warmup_epochs: int = 5):

    if LR_SCHEDULER == "multistep":
        adjusted_milestones = [m - warmup_epochs if m > warmup_epochs else m for m in MULTISTEP_MILESTONES]
        ...
        def lr_lambda(epoch):
            if epoch < warmup_epochs:
                ...
                decay_count = sum(1 for m in adjusted_milestones if current_epoch >= m)
        ...
        def lr_lambda(epoch):
            if epoch < warmup_epochs:
                ...
                return 0.5 * (1 + torch.cos(torch.tensor(
                    torch.pi * (epoch - warmup_epochs) / (num_epochs - warmup_epochs)
                )))
...
def main():
    parser = argparse.ArgumentParser(description='Train ResNet for 100-class classification')
    parser.add_argument('--model', type=str, default='ecaresnet50d',
                       choices=['resnet50_inat', 'resnext101_32x8d', 'resnet101', 'ecaresnet50d', 'gcresnet50t'],
                       help='Model options: resnet50_inat, resnext101_32x8d, resnet101, ecaresnet50d, gcresnet50t')
    ...
    if args.device == 'cuda':
        try:
            ...
            print(f" CUDA incompatibility detected")
            print("→ Switching to CPU training (compatible with all Pwide_resnet50_2yTorch features)")
            args.device = 'cpu'
    ...
    logger.info(f"  - Optimizer: {OPTIMIZER.upper()} (momentum={MOMENTUM}, weight_decay={WEIGHT_DECAY})")
    logger.info(f"  - Scheduler: {LR_SCHEDULER.upper()} (T_max={COSINE_T_MAX}, eta_min={COSINE_ETA_MIN})")
    logger.info(f"  - Warmup Epochs: {WARMUP_EPOCHS}")
    logger.info(f"  - Max Epochs: {args.epochs}")
    logger.info(f"  - Loss Function: {LOSS_FUNCTION.upper()} (label_smoothing={LABEL_SMOOTHING})")
    logger.info(f"  - Early Stopping Patience: {EARLY_STOPPING_PATIENCE}")
    logger.info(f"\nData Augmentation:")
    logger.info(f"  - Image Size: {IMAGE_SIZE}x{IMAGE_SIZE}")
    logger.info(f"  - Random Flip: {TRAIN_TRANSFORMS.get('random_flip', False)}")
    logger.info(f"  - Color Jitter: {TRAIN_TRANSFORMS.get('color_jitter', None)}")
    logger.info(f"  - Random Rotation: {TRAIN_TRANSFORMS.get('random_rotation', 0)}°")
    logger.info(f"  - Random Erasing: {TRAIN_TRANSFORMS.get('random_erasing', None)}")
    logger.info("="*70)
    ...
    logger.info(f"\nDataset:")
    logger.info(f"  - Train: {len(train_loader.dataset)} images ({len(train_loader)} batches)")
    logger.info(f"  - Val: {len(val_loader.dataset)} images ({len(val_loader)} batches)")
    logger.info(f"  - Test: {len(test_loader.dataset)} images ({len(test_loader)} batches)\n")
    
    optimizer = get_optimizer(model, learning_rate=args.lr)
    scheduler = get_scheduler(optimizer, num_epochs=args.epochs, warmup_epochs=WARMUP_EPOCHS)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if classifier.should_freeze_backbone and epoch == classifier.freeze_epochs:
            logger.info(f"\n🔓 STAGE 2: Unfreezing backbone at epoch {epoch + 1}")
            logger.info(f"Switching from fc-only training to full model fine-tuning")
            classifier.unfreeze_backbone()
            
            logger.info(f"Recommended LR for Stage 2: 1e-5 for backbone, {args.lr} for fc layer")
        
        train_loss, train_acc1, train_acc5 = train_epoch(
            model, train_loader, optimizer, criterion, args.device, epoch + 1, logger
        )
        
        val_loss, val_acc1, val_acc5 = validate(model, val_loader, criterion, args.device, logger)
        ...
        if val_acc1 > best_acc:
            ...
            logger.info(f"New best model saved at {save_path} with Acc@1: {best_acc:.2f}%")
            no_improve_count = 0
